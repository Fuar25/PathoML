...
对不同染色的 MIL 模型的预测概率/标签进行结果级跨染色融合，
不修改核心库，仅作为外挂脚本独立运行。

注意：此脚本已从核心库移出，作为实验性功能维护。
"""
...
from scripts.utils.fusion_helpers import (
...
def fuse_binary(big_df, strategy, weights):
    ...
    Args:
        big_df: 包含所有染色预测结果的 DataFrame，需含 patient_id、
                prob_positive、prediction、weight、stain 列。
        strategy: 融合策略，``"majority"`` 为硬投票，``"average"`` 为软投票。
        weights: 各染色权重字典（本函数内权重已内嵌在 big_df["weight"] 列中）。

    Returns:
        Tuple[pd.DataFrame, float, float]: 融合结果 DataFrame、患者级准确率、AUC。
    """
    rows = []
...
def fuse_multiclass(big_df, strategy, weights):
    ...
    Args:
        big_df: 包含所有染色预测结果的 DataFrame，需含 patient_id、
                prob_class_* 列、prediction、weight、stain 列。
        strategy: 融合策略，``"majority"`` 为硬投票，``"average"`` 为软投票。
        weights: 各染色权重字典（本函数内权重已内嵌在 big_df["weight"] 列中）。

    Returns:
        Tuple[pd.DataFrame, float, float]: 融合结果 DataFrame、患者级准确率、macro AUC（OvR）。
    """
...
def fuse_per_fold(stain_runs, k_folds, fusion_dir, strategy):
    """逐折读取各染色的患者级预测 CSV，执行跨染色融合并汇总指标。

    Args:
        stain_runs: 染色配置列表，每项包含 ``name``、``save_dir``、``weight`` 字段。
        k_folds: 交叉验证折数。
        fusion_dir: 融合结果的输出目录。
        strategy: 融合策略，``"majority"`` 或 ``"average"``。

    Returns:
        Tuple[float, float, float, float]: mean_auc、std_auc、mean_acc、std_acc。
    """
...
def main():
    """主流程：依次对 STAIN_RUNS 中各染色运行交叉验证，再执行跨染色融合。

    Returns:
        ``fuse_per_fold`` 的返回值：(mean_auc, std_auc, mean_acc, std_acc)。
    """
