...
_PATIENT_ID_PATTERN = r"((?:xs)?B\d{4}-\d{5})"
...
def _build_key_map(
  ...
  for class_name in os.listdir(root):
    ...
    for fname in os.listdir(class_dir):
      ...
      if key is not None:
        key_map[key] = (class_name, os.path.join(class_dir, fname))
  return key_map
...
class DistillationDataset(BaseDataset):
  ...
  def __init__(
    ...
    patient_id_pattern: str = _PATIENT_ID_PATTERN,
  ) -> None:
    """
    Args:
    ...
    # (1) 扫描各根目录，建立 (patient_id, tissue_id) → (class, path) 映射
    patch_map = _build_key_map(patch_root, patient_id_pattern)
    slide_maps: Dict[str, Dict] = {
      stain: _build_key_map(root, patient_id_pattern)
      for stain, root in slide_roots.items()
    }

    # (2) 确定类别标签（字母序，MALT=0, Reactive=1）
    ...
    for stain, smap in slide_maps.items():
      common_keys &= set(smap)
    if not common_keys:
      counts = {
        'patch': len(patch_map),
        **{stain: len(smap) for stain, smap in slide_maps.items()},
      }
      raise FileNotFoundError(
        f"各根目录无公共样本 ({counts})。请检查路径与文件命名是否一致。"
      )
    ...
    for key in sorted(common_keys):
      ...
      self.samples.append({
        ...
        'slide_paths':  slide_paths,  # Dict[stain, path]
      })
